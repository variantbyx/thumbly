...
export const generateThumbnail = async (req, res) => {
    try {
        ...
        try {
            ...
            if (isQuotaError) {
                const fallbackUrl = fallbackImages[style] || fallbackImages["Bold & Graphic"];
                try {
        ...
        if (hasCloudinaryKeys) {
            ...
            const uploadResult = await new Promise((resolve, reject) => {
                const stream = cloudinary.uploader.upload_stream({ resource_type: "image", format: "png" }, (error, result) => {
                    if (error)
                        reject(error);
                    else if (result)
                        resolve(result);
